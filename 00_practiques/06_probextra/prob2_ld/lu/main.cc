...
int main(int argc, char *argv[]) {
  ...
  if (verbose) cout << "Fitxer d'entrada: " << argv[1] << "\nDimensio: " << n << "\n\n";
  ...
  if (verbose) cout << "Comencem a calcular LU\n\n";
  ...
  if (verbose) cout << "Normes de Ax-b: norma_1 = " << norma_1(residu_2, n) << ", norma_inf = " << norma_inf(residu_2, n) << "\n\n";
  ...
  if (verbose) cout << "Normes de A*A^(-1) - Id: norma_1 = " << norma_m1(Id, n) << ", norma_inf = " << norma_minf(Id, n) << "\n";

  if (verbose) cout << "Nombres de condicionament d'A: K_1=" << condition_number_1(A, inv_A, n) << ", K_inf=" << condition_number_inf(A, inv_A, n) << "\n";


  double** L = create_pointer(n, n);
  double** U = create_pointer(n, n);
  ...
  double** inv_L = copia_punter(L, n, n);
  double** inv_U = copia_punter(U, n, n);
  double** U2 = copia_punter(U, n, n);
  double det_L;
  double det_U;
  inversa(inv_L, inv_L, n, &det_L, TOL);
  inversa(U2, inv_U, n, &det_U, TOL);

  if (verbose) cout << "Nombres de condicionament de L: K_1=" << condition_number_1(L, inv_L, n) << ", K_inf=" << condition_number_inf(L, inv_L, n) << "\n";
  if (verbose) cout << "Nombres de condicionament de U: K_1=" << condition_number_1(U, inv_U, n) << ", K_inf=" << condition_number_inf(U, inv_U, n) << "\n";

  if (verbose) {
    cout << "Solucio (vector x): ";
    print_vector(x, n);
  }

  double* x_diferencia = copia_punter(x, n);
  for (int i = 0; i < n; ++i) x_diferencia[i] -= 1;

  double En = norma_inf(x_diferencia, n);

  if (verbose) cout << "E_n: " << En << "\n";
  else cout << En << "\n" << condition_number_inf(A, inv_A, n) << "\n";
}
